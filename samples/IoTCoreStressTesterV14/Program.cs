namespace IoTCoreStressTesterV14
{
    using System;
    ...
    internal class Program
    {
        ...
        private static void Run(string id, string httpUri, int cacheTimeout, int pollInterval, string mirror1Uri, string mirror2Uri, string mirror3Uri, string mirror4Uri)
        {
            ...
            // Create IoTCore
            var ioTCore = IoTCoreFactory.Create(id, null);
            ...
            {
                deviceManager.Mirror(new MirrorRequestServiceData(mirror1Uri, "mirror1", cacheTimeout));
            }
            if (!string.IsNullOrEmpty(mirror2Uri))
            {
                deviceManager.Mirror(new MirrorRequestServiceData(mirror2Uri, "mirror2", cacheTimeout));
            }
            if (!string.IsNullOrEmpty(mirror3Uri))
            {
                deviceManager.Mirror(new MirrorRequestServiceData(mirror3Uri, "mirror3", cacheTimeout));
            }
            if (!string.IsNullOrEmpty(mirror4Uri))
            {
                deviceManager.Mirror(new MirrorRequestServiceData(mirror4Uri, "mirror4", cacheTimeout));
            }
            Console.WriteLine($"Mirroring takes {stopWatch.ElapsedMilliseconds} ms");

            // Create iodd extender
            stopWatch.Restart();
            var ioddFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "ifm", "iodds");
            var ioddExtender = new IoddExtenderProfileBuilder(ioTCore, IoddLoaderDefault.GetInstance(ioddFolder));
            Console.WriteLine($"Creating iodd extender and loading iodds takes {stopWatch.ElapsedMilliseconds} ms");

            // Run iodd extender
            stopWatch.Restart();
            ioddExtender.Extend();
            Console.WriteLine($"Running iodd extender takes {stopWatch.ElapsedMilliseconds} ms");
